...
```bash
git clone https://github.com/ApurvaDabhade/SmartHire-AI.git
cd SmartHire-AI
...
Add your [Groq API key](https://console.groq.com/keys) to `.env` as `GROQ_API_KEY`.
...
1. Confirm the API key in the sidebar (loaded from `.env`).
2. Leave **RAG Mode** on Generic RAG, or choose RAG Fusion for complex job ads.
3. Type a **job description** in the chat to retrieve similar resumes, or ask about a specific **Applicant ID**.
4. Optional: upload your own resumes as a **CSV**. Preferred columns are `Resume` and `ID`; other common layouts are mapped automatically. Uploaded data replaces the default pool for the current session only.
